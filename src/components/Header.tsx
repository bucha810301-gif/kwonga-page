...
import { LogOut, Search, Plus, User, Users, ChevronLeft, ChevronRight, LayoutList, GitFork, ChevronDown } from 'lucide-react';
...
export function Header({ userProfile, onLogout, onSearch, onAddMember, hasMembers, memberCount = 0, searchResultCount, currentResultIndex, onNextResult, onPrevResult, view = 'tree', onViewChange, isCollapsed = false, onExpand }: HeaderProps) {
  ...
  return (
    ...
    >
      ...
      {/* ── MOBILE COLLAPSED: 미니 바 ── */}
      <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isCollapsed ? 'max-h-[48px]' : 'max-h-0'}`}>
        <div className="h-11 flex items-center justify-between px-3">
          {/* 권 로고 */}
          <div className="flex items-center gap-2">
            <div
              className="w-[28px] h-[28px] rounded-[7px] flex items-center justify-center select-none"
              style={{ background: 'linear-gradient(145deg, #c5a059 0%, #e8c87a 45%, #a07830 100%)', boxShadow: '0 1px 6px rgba(197,160,89,0.4)' }}
            >
              <span style={{ fontFamily: '"Noto Serif KR", serif', fontSize: '15px', fontWeight: 700, color: '#fff', lineHeight: 1 }}>權</span>
            </div>
            <span className="text-xs font-semibold text-slate-300">영광종회 가계도</span>
          </div>
          {/* 펼치기 버튼 */}
          <button
            onClick={onExpand}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-slate-300 hover:text-white hover:bg-white/10 transition cursor-pointer"
          >
            <ChevronDown size={14} />
            <span className="text-[11px] font-semibold">메뉴</span>
          </button>
        </div>
      </div>

      {/* ── FULL HEADER (항상 데스크톱, 모바일은 미전개 시) ── */}
      <div className={`overflow-hidden transition-all duration-300 ease-in-out md:max-h-[200px] md:opacity-100 ${isCollapsed ? 'max-h-0 opacity-0' : 'max-h-[200px] opacity-100'}`}>
